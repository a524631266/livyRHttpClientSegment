...
public class RHttpConf extends ClientConf<RHttpConf> {
    ...
    public enum Entry implements ConfEntry{
        ...
        CONNECTION_IDLE_TIMEOUT("connection.idle.timeout", "1m"), // 最多闲置时间10分钟
        ...
        CONNECTION_AUTOCONNECT_INTERVAL("connection.autoconnect.interval", "10s"), // yi
        CONNECTION_SESSION_CORE_SIZE("connection.session.core.size", 1), // 该客户端中最少要保留的session,远程的spark进程数量
        CONNECTION_SESSION_MAX_SIZE("connection.session.max.size", 2) // 该客户端中最多持有的shell数量，防止使用过度
        ;
